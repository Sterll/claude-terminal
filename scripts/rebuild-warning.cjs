...
const lines = [
  ...
  `  ${YELLOW}native module will work. These will not:${OFF}`,
  '',
  '    node-pty        terminals',
  '    keytar          GitHub token, Groq key, account switching',
  '',
  `  ${YELLOW}better-sqlite3 is an optional dependency and was probably dropped${OFF}`,
  `  ${YELLOW}by npm for the same reason, which costs you SQLite connections in${OFF}`,
  `  ${YELLOW}the database panel. The other drivers are pure JavaScript and fine.${OFF}`,
  '',
