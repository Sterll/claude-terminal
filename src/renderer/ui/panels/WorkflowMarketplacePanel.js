...
function _stepText(count) {
  return `${count} ${t(count === 1 ? 'workflow.marketplace.stepSingular' : 'workflow.marketplace.stepPlural')}`;
}
    ...
    <div class="hub-sidebar">
      <div class="hub-sidebar-brand">
        ...
        <div>
          ...
          <div class="hub-brand-sub">${t('workflow.marketplace.community')}</div>
        </div>
      </div>
      ...
      <nav class="hub-nav">
        <button class="hub-nav-item ${st.tab === 'browse' ? 'active' : ''}" data-hubtab="browse">
          ...
          ${t('workflow.marketplace.explore')}
        </button>
        <button class="hub-nav-item ${st.tab === 'mine' ? 'active' : ''}" data-hubtab="mine">
          ...
          ${t('workflow.marketplace.myPublications')}
        </button>
      </nav>
      ...
      <div class="hub-sidebar-footer">
        <button class="hub-publish-btn" id="hub-open-publish">
          ...
          ${t('workflow.marketplace.publishWorkflow')}
        </button>
      </div>
    </div>
    ...
    <div class="hub-main">
      ...
      <div class="hub-topbar">
        <div class="hub-search-wrap">
          ...
          <input class="hub-search" id="hub-search" placeholder="${t('workflow.marketplace.searchPlaceholder')}" value="${escapeHtml(st.query)}">
        ...
        <div class="hub-sort-wrap">
          <span class="hub-sort-label">${t('workflow.marketplace.sort')}</span>
          <div class="hub-sort-pills">
            <button class="hub-sort-pill ${st.sort === 'imports' ? 'active' : ''}" data-hubsort="imports">
              ...
              ${t('workflow.marketplace.sortPopular')}
            </button>
            <button class="hub-sort-pill ${st.sort === 'date' ? 'active' : ''}" data-hubsort="date">
              ...
              ${t('workflow.marketplace.sortRecent')}
            </button>
            <button class="hub-sort-pill ${st.sort === 'name' ? 'active' : ''}" data-hubsort="name">
              ...
              ${t('workflow.marketplace.sortAlpha')}
            </button>
          </div>
        </div>

        <button class="hub-close-btn" id="hub-close" title="${t('common.close')}">
  ...
  if (!items.length) {
    container.innerHTML = `
      <div class="hub-empty">
        ...
        <p>${st.query ? t('workflow.marketplace.noResults', { query: escapeHtml(st.query) }) : t('workflow.marketplace.noWorkflow')}</p>
        ${!st.query ? `<span>${t('workflow.marketplace.beFirst')}</span>` : ''}
      </div>
    `;
    return;
  }
  ...
  return `
    <div class="hub-card" data-id="${escapeHtml(item.id)}" style="animation-delay:${delay}ms">
      ...
      <div class="hub-card-body">
        ...
        <div class="hub-card-author">${t('workflow.marketplace.by')} <strong>${escapeHtml(item.author || t('workflow.marketplace.anonymous'))}</strong></div>
    ...
    <div class="hub-detail">
      <div class="hub-detail-hd">
        ...
        <div class="hub-detail-titles">
          ...
          <div class="hub-detail-by">${t('workflow.marketplace.by')} <strong>${escapeHtml(item.author || t('workflow.marketplace.anonymous'))}</strong></div>
        </div>
      ...
      <div class="hub-detail-body">
        <div class="hub-detail-chips">
          <span class="hub-chip">
            ...
            ${t('workflow.marketplace.imports', { n: _fmt(item.imports || 0) })}
          </span>
          ${item.createdAt ? `<span class="hub-chip">${_relDate(item.createdAt)}</span>` : ''}
        </div>

        <p class="hub-detail-desc">${escapeHtml(item.description || t('common.noDescription'))}</p>

        <div class="hub-tags" style="margin-bottom:18px">${tagsHtml}</div>

        ${item.workflowJson ? `
          <div class="hub-detail-section">${t('workflow.marketplace.workflowPreview')}</div>
      ...
      <div class="hub-detail-ft">
        <button class="hub-detail-cancel" id="hub-det-cancel">${t('common.close')}</button>
        <button class="hub-detail-import" id="hub-det-import">
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
          ${t('workflow.marketplace.importThisWorkflow')}
        </button>
      </div>
    </div>
  `;
...
async function _importWorkflow(item, btn) {
  if (!_ctx?.api?.workflow) { _toast(t('workflow.marketplace.apiUnavailable'), 'error'); return; }
  ...
  try {
    ...
    _toast(t('workflow.marketplace.importedToast', { name: item.name }), 'success');
  ...
  if (!st.mine.length) {
    container.innerHTML = `
      <div class="hub-empty">
        ...
        <p>${t('workflow.marketplace.mineEmptyTitle')}</p>
        <span>${t('workflow.marketplace.mineEmptySub')}</span>
      </div>
    `;
    return;
  }
  ...
  overlay.innerHTML = `
    <div class="hub-pub-modal">
      <div class="hub-pub-hd">
        <span class="hub-pub-title">${t('workflow.marketplace.publishWorkflow')}</span>
      ...
      <div class="hub-pub-body">
        ...
        <div class="hub-pub-field">
          <label class="hub-pub-label">${t('workflow.marketplace.workflowToPublish')}</label>
          <div class="hub-custom-select" id="hub-wf-select">
            <div class="hub-custom-select-trigger" id="hub-wf-trigger">
              <span id="hub-wf-selected-label">${t('workflow.marketplace.chooseWorkflow')}</span>
              ...
              ${workflows.length
                ? workflows.map(w => `
                    <div class="hub-custom-select-option" data-wfid="${escapeHtml(w.id)}" data-wfname="${escapeHtml(w.name || w.id)}">
                      ...
                      <div class="hub-custom-select-option-meta">${_stepText((w.steps || []).length)}</div>
                    </div>`).join('')
                : `<div class="hub-custom-select-empty">${t('workflow.marketplace.noWorkflowFound')}</div>`
              }
            </div>
          </div>
        ...
        <div class="hub-pub-field">
          <label class="hub-pub-label">${t('workflow.marketplace.descriptionLabel')} <span class="hub-pub-hint">${t('workflow.marketplace.publicHint')}</span></label>
          <textarea class="hub-pub-textarea" id="hub-pub-desc" rows="3" placeholder="${t('workflow.marketplace.descriptionPlaceholder')}"></textarea>
          <span class="hub-pub-char" id="hub-pub-char">0 / 500</span>
        </div>

        <div class="hub-pub-field">
          <label class="hub-pub-label">${t('workflow.marketplace.tagsLabel')} <span class="hub-pub-hint">${t('workflow.marketplace.tagsHint')}</span></label>
        ...
        <div class="hub-pub-field">
          <label class="hub-pub-label">${t('workflow.marketplace.authorLabel')} <span class="hub-pub-hint">${t('workflow.marketplace.publicHint')}</span></label>
          <input class="hub-pub-input" id="hub-pub-author" placeholder="${t('workflow.marketplace.authorPlaceholder')}" value="${escapeHtml(_loadAuthor())}">
        </div>

        <div class="hub-pub-info">
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
          ${t('workflow.marketplace.publishInfo')}
        </div>
      </div>

      <div class="hub-pub-ft">
        <button class="hub-pub-cancel" id="hub-pub-cancel">${t('common.cancel')}</button>
        <button class="hub-pub-submit" id="hub-pub-submit">
          ...
          ${t('workflow.marketplace.publish')}
        </button>
      </div>
    </div>
  `;
...
async function _submitPublish(overlay) {
  ...
  const author = overlay.querySelector('#hub-pub-author')?.value?.trim() || t('workflow.marketplace.anonymous');

  if (!wfId) { _toast(t('workflow.marketplace.chooseWorkflowError'), 'error'); return; }
  if (!desc) { _toast(t('workflow.marketplace.addDescriptionError'), 'error'); return; }
  if (desc.length > 500) { _toast(t('workflow.marketplace.descriptionTooLong'), 'error'); return; }
  ...
  btn.disabled = true;
  btn.innerHTML = `<span class="hub-spinner"></span> ${t('workflow.marketplace.publishing')}`;
  ...
  if (HUB_URL) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        _toast(t('workflow.marketplace.publishedToast', { name: wfName }), 'success');
      } else {
        _toast(t('workflow.marketplace.publishError'), 'error');
      }
    } catch {
      _toast(t('workflow.marketplace.offlineQueued'), 'error');
    }
  } else {
    _toast(t('workflow.marketplace.hubUnavailable'), 'error');
  }
}
...
function _relDate(iso) {
  try {
    ...
    if (diff < 86400000) return t('workflow.marketplace.today');
    if (diff < 604800000) return t('workflow.marketplace.daysAgoShort', { n: Math.floor(diff / 86400000) });
    if (diff < 2592000000) return t('workflow.marketplace.weeksAgoShort', { n: Math.floor(diff / 604800000) });
    const locale = _isFr() ? 'fr-FR' : 'en-US';
